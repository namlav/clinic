...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Khám phá", style: TextStyle(color: Colors.black, fontWeight: FontWeight.w700)),
      ),
      body: Padding(
                  ...
                  return Padding(
                    ...
                    child: ChoiceChip(
                      ...
                      shape: RoundedRectangleBorder(
                        ...
                        side: BorderSide(
                          color: isSelected ? const Color(0xFF0057C2) : Colors.grey.shade300,
                        ),
                      ),
                    ),
                  );
  ...
  Widget _buildDoctorsList() {
    if (filteredDoctors.isEmpty) {
      return const Center(
        child: Text("Không tìm thấy bác sĩ nào phù hợp", style: TextStyle(color: Color(0xFF6E7688))),
      );
    }
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final String title = doc['specialties']?['specialtyname'] ?? doc['title'] ?? "Chuyên khoa";
        final String avatarUrl = doc['avatarurl'] ?? "";

        return GestureDetector(
          onTap: () => Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => DoctorProfilePage(doctorData: doc)),
          ),
          child: Container(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(fullname, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Color(0xFF1A1F36))),
                      const SizedBox(height: 4),
                      Text(title, style: const TextStyle(fontSize: 13, color: Color(0xFF6E7688))),
                    ],
                  ),
                ),
                const Icon(Icons.arrow_forward_ios, size: 14, color: Color(0xFFB5BDCA)),
              ],
            ),
          ),
        );
  ...
  Widget _buildSpecialtiesList() {
    if (filteredSpecialties.isEmpty) {
      return const Center(
        child: Text("Không tìm thấy chuyên khoa nào phù hợp", style: TextStyle(color: Color(0xFF6E7688))),
      );
    }
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return GestureDetector(
          onTap: () => Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => SpecialtyDoctorsScreen(specialty: spec)),
          ),
          child: Container(
            margin: const EdgeInsets.only(bottom: 12),
            padding: const EdgeInsets.all(16),
            ...
            child: Row(
              children: [
                Container(
                  ...
                  child: const Icon(Icons.local_hospital, color: Color(0xFF0057C2)),
                ),
                const SizedBox(width: 14),
                Expanded(
                  child: Text(name, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Color(0xFF1A1F36))),
                ),
                const Icon(Icons.arrow_forward_ios, size: 14, color: Color(0xFFB5BDCA)),
              ],
            ),
  ...
  Widget _buildServicesList() {
    if (filteredServices.isEmpty) {
      return const Center(
        child: Text("Không tìm thấy dịch vụ nào phù hợp", style: TextStyle(color: Color(0xFF6E7688))),
      );
    }
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final String priceStr = (serv['price'] ?? 0).toString().replaceAll(RegExp(r'\.0*$'), '');
        ...
        return GestureDetector(
          onTap: () {
            if (specialty != null) {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => SpecialtyDoctorsScreen(specialty: specialty)),
              );
            }
          },
          child: Container(
            ...
            child: Row(
              children: [
                Container(
                  ...
                  height: 44,
                  decoration: BoxDecoration(color: const Color(0xFFEAF2FF), borderRadius: BorderRadius.circular(10)),
                  child: const Icon(Icons.medical_services, color: Color(0xFF0057C2)),
                ),
                const SizedBox(width: 14),
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(name, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: Color(0xFF1A1F36))),
                      const SizedBox(height: 4),
                      Text(specialty?['specialtyname'] ?? 'Chuyên khoa', style: const TextStyle(fontSize: 13, color: Color(0xFF6E7688))),
                    ],
                  ),
                ),
                Text("${priceStr}đ", style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: Color(0xFF0057C2))),
              ],
            ),
          ),
        );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(widget.specialty['specialtyname'] ?? 'Bác sĩ',
            style: const TextStyle(color: Colors.black, fontWeight: FontWeight.w700)),
      ),
      body: isLoading
          ? const Center(child: CircularProgressIndicator(color: Color(0xFF0057C2)))
          : doctors.isEmpty
              ? const Center(child: Text("Không có bác sĩ nào trong chuyên khoa này", style: TextStyle(color: Color(0xFF6E7688))))
              : ListView.builder(
